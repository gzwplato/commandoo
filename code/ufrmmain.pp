...
type
  ...
  TfrmMain = class(TForm)
    ...
    procedure btnTestMeClick(Sender: TObject);
...
//update INCREMENT c_PROG_VersionUpgradeCount = #; to the next upgrade if prog upgrades were necessary
//       cHandwrittenVersion = '0.1.3'; to match readme file
...
//=============== first time run as superuser is NOT ALLOWED. ========================================
  ...
  fFavoritesSO.Searches[ cIdxCmd ].AddSearchItem( fidIsFavorite, dbsTrueStr, coEqual );
  fFavoritesSO.Searches[ cIdxCmdLine ].AddSearchItem( fidIsFavorite, dbsTrueStr, coEqual );
...
begin
  ...
  lblCLBlink.Visible := lblCEditing.Visible;
  lblCBlink.Visible := not lblCEditing.Visible;
end;
...
procedure TfrmMain.btnTestMeClick(Sender: TObject);
begin
  showmessage( 'no tests implemented right now' );
end;
...
begin
  ...
  sbStatuses.Panels[ 2 ].Text := ' ' + cmsgHelpVersionInfoVersion + ':  ' + cHandwrittenVersion + cVersionDate + '  ';
end;
