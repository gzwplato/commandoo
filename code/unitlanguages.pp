...
type
  ...
  public
    ...
    procedure UpdateProgramLang( const Lang : string );
...
//    WriteString( fSectTabLanguages, cLangAbbrev_calif, cLangStr_calif );
  ...
  if fProgramLang = '' then
    fProgramLang := ChooseLanguage;

  UpdateProgramLang( fProgramLang );
...
procedure TLanguages.UpdateProgramLang( const Lang : string );
var
  POFileName : string;
begin
...
function TLanguages.ChooseLanguage : string;
begin
