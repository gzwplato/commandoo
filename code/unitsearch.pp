...
//both combined in search logic. I think I have a way to do that but will let that wait until after first off. release
  ...
  public
    ...
    procedure AddSearchItem( const CmdIdx : integer; const SearchVal : string;
                                         const Cond : TCondition; const CaseSens : boolean = false ); overload;
    procedure AddSearchItem( const CmdIdx : TCmdFieldID; const SearchVal : string;
                             const Cond : TCondition; const CaseSens : boolean = false ); overload;
...
procedure TBaseSearch.AddSearchItem( const CmdIdx : integer; const SearchVal : string;
                                     const Cond : TCondition; const CaseSens : boolean = false );
var
  Idx : Integer;
begin
  Idx := SIList.Add( TSearchItem.Create( Self, CmdStruct[ CmdIdx ].FieldType ) );

  TSearchItem( SIList[ Idx ] ).SearchItem_Init( SearchVal, Cond, CaseSens, CmdStruct[ CmdIdx ].UniqueID );

end;

procedure TBaseSearch.AddSearchItem( const CmdIdx : TCmdFieldID; const SearchVal : string;
                                     const Cond : TCondition; const CaseSens : boolean );
begin
  AddSearchItem( Ord( CmdIdx ), SearchVal, Cond, CaseSens );
end;
